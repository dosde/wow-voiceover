...
---@field Language string Language of the recordings (TOC ##X-VoiceOver-DataModule-Language, "enUS" if missing)
...
function DataModules:GetModules()
    -- Optionally ignore recordings that are not in the client's language, so those lines fall
    -- back to text-to-speech and get collected for generation in the right language
    local profile = Addon.db and Addon.db.profile.TTS
    if profile and profile.MatchLanguage and GetLocale then
        local locale = GetLocale()
        local filtered = {}
        for _, module in ipairs(self.registeredModulesOrdered) do
            if module.METADATA.Language == locale then
    ...
    for i = 1, GetNumAddOns() do
        ...
        if moduleVersion and (FORCE_ENABLE_DISABLED_MODULES or GetAddOnEnableState(playerName, i) ~= 0) then
            ...
            local module =
            {
                ...
                Maps = maps,
                Language = GetAddOnMetadata(name, "X-VoiceOver-DataModule-Language") or "enUS",
            }
